...
namespace PathBerserker2d
{
    internal class IntersectionTester
    {
        ...
        public void MarkContour(Contour contour, NavSurface owner, PolygonSet polySet, List<NavSegment> navSegments)
        {
            ...
            {
                for (int i = 0, j = contour.Verts.Count - 1; i < contour.VertexCount; j = i, i++)
                {
                    Gizmos.color = Color.red;
                    Gizmos.DrawLine(CIntConversion.IntPointToVector2(contour.Verts[i]), CIntConversion.IntPointToVector2(contour.Verts[j]));
                }
